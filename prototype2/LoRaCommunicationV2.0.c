	...
	void receivepacket() {
		...
		if(digitalRead(dio0) == 1)
		{
			if(receive(message)) {
				...
				strftime(filename, sizeof(filename), "/home/pi/EncryptedFiles/%Y-%m-%d_%H:%M:%S_Encrypted.txt", timenow);
	...
	int main (int argc, char *argv[]) {
		...
		if (!strcmp("sender", argv[1])) 
		{
			...
			FILE * file = fopen("/home/pi/Hello.txt", "r+");
			// if (file == NULL) return;
			...
			file = fopen("/home/pi/Hello.txt", "r+");
