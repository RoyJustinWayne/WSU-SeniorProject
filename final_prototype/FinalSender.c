	...
	void receivepacket() {
		...
		if(digitalRead(dio0) == 1)
		{
			...
			if(receive(message)) {
				...
				strftime(filename, sizeof(filename), "/home/pi/EncryptedFilesReceived/%Y-%m-%d_%H:%M:%S_Encrypted.txt", timenow);
				//2018-10-25_20-24-30_Encrypted.txt
	...
	int main (int argc, char *argv[]) {
		...
		//{			
			...
			//This can be changed, suggested power between 3-20 but TA Dali suggested we can try going as high as 25-30
			configPower(23);
			//Output to terminal.
			...
			{
				//Justin/Naim 11-6-2018
				//Using delay to make sure data is written in the place we are looking for.
				delay(20000);
				ftw("/home/pi/Desktop/weather-station-master/client/data", checkifnewer, 1);
				...
				//Final writing to a buffered packet, delay can be changed to send messages at a reduced rate.
				txlora(hello, strlen((char *)hello)-8);
				delay(5000);
			}
